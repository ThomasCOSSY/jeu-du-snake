...
function game() {
    // coordonnée de la tête du serpent
    px += xv;
    py += yv;
    // si il touche un bord, il apparait de l autre coté
    if (px < 0) {
        px = tc - 1;
    }
    if (px > tc - 1) {
...
// toile de fond noir, ecran de jeu
    ctx.fillStyle = 'black';
    ctx.fillRect(0, 0, canvas.width, canvas.height);
// le serpent, en lime (vert fluo)
    ctx.fillStyle = 'lime';
    ...
    // la pomme rouge la fonction pour rendre sa position aleatoire est juste au dessus
    ctx.fillStyle = 'red';
